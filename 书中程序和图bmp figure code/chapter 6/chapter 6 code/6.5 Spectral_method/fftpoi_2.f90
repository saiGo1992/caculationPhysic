...
! Periodic boundary conditions
      parameter( Nmax = 32,npmax=1000 )
      real L,x(Nmax),y(Nmax),rho(Nmax,Nmax),phi(Nmax,Nmax)
      real cx(Nmax),cy(Nmax),P(Nmax,Nmax),rx(npmax),ry(npmax)
	  ...
	  M=100
      call positions(M,rx,ry,L)      ! 将M个电荷放到圆周上
	  q=1.0
      do i=1,M
        ii = int(rx(i)/h+0.5)+1      ! Place charge at nearest
		 if (ii>N) ii=ii-N  
        jj = int(ry(i)/h+0.5)+1      ! grid point
		 if (jj>N) jj=jj-N 
...
! 
      ...
      stop
 1001 format(e12.6)
 1002 format(e12.6,' ',$)   ! The $ suppresses the carriage return
 1003 format(/)             ! New line
      end

subroutine positions(m,x,y,L)
real x(m),y(m),L
dx=6.28/m
do i=1,m
  x(i)=L/2.-L/2.*sin(dx*i)
  y(i)=L/2.-L/2.*cos(dx*i)
end do
end
